...
namespace PointOfSaleSystem
{
    public partial class Reports : Form
    {
        ...
        private void Reports_Load(object sender, EventArgs e)
        {
            ...
            ShowLedgersInfo(DGVLedgersReports, CustomerLedgersInfoIDGV, CustomerLedgerIDGV,CustomerGV, PayingDateGV, LedgerInvoiceNoGV, LedgerTotalGv, PreviousPaidGV, TodaysPaidGV, NewBalanceGV);
        ...
        private void ShowLedgersInfo(DataGridView dgv, DataGridViewColumn InfoID, DataGridViewColumn LedgerID,
            DataGridViewColumn Customer, DataGridViewColumn PayingDate, DataGridViewColumn InvoiceNo,
            DataGridViewColumn TotalAmount, DataGridViewColumn PreviousPaid, DataGridViewColumn TodayPaid, DataGridViewColumn NewBalance, string search = null)
        {
            try
            {
                ...
                {
                    cmd = new SqlCommand("select ct.CustomerLedgerInfoID,ct.CustomerLedger_ID,p.Name,ct.PayingDate,ct.InvoiceNo,ct.TotalAmount,ct.PreviousPaid,ct.TodayPaid,ct.NewBalance  from CustomerLedgersInfoTable ct inner join PersonsTable p on p.PersonID = ct.Customer_ID", MainClass.con);
                }
                else
                {
                    cmd = new SqlCommand("select ct.CustomerLedgerInfoID,ct.CustomerLedger_ID,p.Name,ct.PayingDate,ct.InvoiceNo,ct.TotalAmount,ct.PreviousPaid,ct.TodayPaid,ct.NewBalance  from CustomerLedgersInfoTable ct inner join PersonsTable p on p.PersonID = ct.Customer_ID where ct.Customer_ID = '"+search+"'", MainClass.con);
                }
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                da.Fill(dt);
                InfoID.DataPropertyName = dt.Columns["CustomerLedgerInfoID"].ToString();
        ...
        private void ShowRestaurantSales(DataGridView dgv, DataGridViewColumn SaleID, DataGridViewColumn InvoiceNo, DataGridViewColumn SaleDate, DataGridViewColumn OrderType, DataGridViewColumn SaleTime, DataGridViewColumn GrandTotal, string search = null)
        {
            ...
            if (saledate == 1)
            {
                ...
                {
                    cmd = new SqlCommand("select SaleID,InvoiceNo,format(SaleDate, 'dd/MM/yyyy') as 'Date',OrderType, SaleTime,GrandTotal  from SalesTable where SaleDate between '" + dtSale1.Value.ToShortDateString() + "' and '" + dtSale2.Value.ToShortDateString() + "' ", MainClass.con);
                }
                else
                {
                    cmd = new SqlCommand("select SaleID,InvoiceNo,format(SaleDate, 'dd/MM/yyyy') as 'Date',OrderType, SaleTime,GrandTotal  from SalesTable where SaleDate between '" + dtSale1.Value.ToShortDateString() + "' and '" + dtSale2.Value.ToShortDateString() + "' and InvoiceNo  like '%"+search+"%' ", MainClass.con);
                }
            }
            else
            {
                if(search == "" || search == null)
                {
                    cmd = new SqlCommand("select SaleID,InvoiceNo,format(SaleDate, 'dd/MM/yyyy') as 'Date',OrderType, SaleTime,GrandTotal  from SalesTable", MainClass.con);
                }
                else
                {
                    cmd = new SqlCommand("select SaleID,InvoiceNo,format(SaleDate, 'dd/MM/yyyy') as 'Date',OrderType, SaleTime,GrandTotal  from SalesTable where InvoiceNo  like '%" + search + "%'", MainClass.con);
                }
            }
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
        ...
        {
            ShowLedgersInfo(DGVLedgersReports, CustomerLedgersInfoIDGV, CustomerLedgerIDGV, CustomerLedgerIDGV, PayingDateGV, LedgerInvoiceNoGV, LedgerTotalGv, PreviousPaidGV, TodaysPaidGV, NewBalanceGV,CboCustomerLedger.SelectedValue.ToString());
